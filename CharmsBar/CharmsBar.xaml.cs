   ...
   private const int MOUSEEVENTF_RIGHTUP = 0x10;
    ...
    const int MIN_ALL_UNDO = 416; 
        ...
        private void SetBackgroundColor()
        {
            try
            {
                var brush = (Brush)new BrushConverter().ConvertFromString("#FF009688"); // Teal
                MetroColor.Background = brush;
                MetroColor.Opacity = 1.0;
                MetroColor.Visibility = Visibility.Visible;
            }
            catch
            {
                MetroColor.Background = Brushes.Gray;
            }
        }
        ...
        private void OnTimedEvent(object sender, ElapsedEventArgs e)
        {
            dispatcher.BeginInvoke((Action)(() =>
            {
                ...
                var lang = (ci.TwoLetterISOLanguageName ?? "").ToLowerInvariant();
                var name = (ci.Name ?? "").ToLowerInvariant();

                if (lang == "ja" || name.StartsWith("ja")) // Japanese
                {
                ...
                else if (lang == "de" || name.StartsWith("de")) // German
                {
                ...
                else if (lang == "fr" || name.StartsWith("fr")) // French
                {
                    SearchText.Content = "Rechercher";
                    ShareText.Content = "Partager";
                    WinText.Content = "Démarrer";
                    DevicesText.Content = "Appareils";
                    SettingsText.Content = "Paramètres";
                }
                else if (lang == "es" || name.StartsWith("es")) // Spanish
                {
                    SearchText.Content = "Buscar";
                    ShareText.Content = "Compartir";
                    WinText.Content = "Inicio";
                    DevicesText.Content = "Dispositivos";
                    SettingsText.Content = "Configuración";
                }
                else if (lang == "zh" || name.StartsWith("zh")) // Chinese (simplified/traditional)
                {
                    SearchText.Content = "搜索";
                    ShareText.Content = "共享";
                    WinText.Content = "开始";
                    DevicesText.Content = "设备";
                    SettingsText.Content = "设置";
                }
                else if (lang == "ar" || name.StartsWith("ar")) // Arabic
                {
                    //to do: add rtl support
                    SearchText.Content = "بحث";
                    ShareText.Content = "مشاركة";
                    WinText.Content = "ابدأ";
                    DevicesText.Content = "الأجهزة";
                    SettingsText.Content = "الإعدادات";
                }
                else if (lang == "pt" || name.StartsWith("pt")) // Portuguese
                {
                    SearchText.Content = "Pesquisar";
                    ShareText.Content = "Partilhar";
                    WinText.Content = "Iniciar";
                    DevicesText.Content = "Dispositivos";
                    SettingsText.Content = "Configurações";
                }
                else if (lang == "ru" || name.StartsWith("ru")) // Russian
                {
                    SearchText.Content = "Поиск";
                    ShareText.Content = "Поделиться";
                    WinText.Content = "Пуск";
                    DevicesText.Content = "Устройства";
                    SettingsText.Content = "Параметры";
                }
                else
                {
                    // keep defaults (English)
                    SearchText.Content = "Search";
                    ShareText.Content = "Share";
                    WinText.Content = "Start";
                    DevicesText.Content = "Devices";
                    SettingsText.Content = "Settings";
                }
                if (findTimer > 50)
                {
